...
/** Centre of the CANVAS-space Soldiers button (menu-local {354,562,92,22}). */
const SOLDIERS_CLICK = { x: MENU_X + 354 + 46, y: MENU_Y + 562 + 11, button: 0 as const };

describe('menu BottomStrip Soldiers routing (item 040)', () => {
  it('reports soldiers: false while hidden and true once the strip shows Soldiers', () => {
    const off = new BottomStrip({});
    const on = new BottomStrip({ soldiers: true });
  ...
  it('keeps the Soldiers button inert when the click lands without the flag even if the pointer hovers it', () => {
    const off = new BottomStrip({ soldiers: false });
    const clickInput = menuInput({ clicks: [SOLDIERS_CLICK], mouse: { x: 354 + 46, y: 562 + 11 } });
    expect(off.update(clickInput).soldiers).toBe(false);
  });
});
...
describe('RosterScreen (item 040)', () => {
  ...
  it('strip Back routes back exactly like ESC', () => {
    game.campaign = campaignWithTeams();
    const saved = game.screen;
    const fakeBack: Screen = { update() {}, draw() {}, cursor() { return 'arrow'; } };
    game.setScreen(new RosterScreen(fakeBack));
    // click the Back button (canvas coords over the menu-local rect {16, 562, 92, 22})
    game.screen.update(1 / 60, inputState({ clicks: [{ x: MENU_X + 16 + 46, y: MENU_Y + 562 + 11, button: 0 }], mouse: { x: 0, y: 0 } }));
    expect(game.screen).toBe(fakeBack);
    game.screen = saved;
    game.campaign = null;
  });
...
describe('roster entry-point wiring (item 040)', () => {
  it('operation briefing-mode strip enables Soldiers and COA/debrief strips enable it with a campaign', async () => {
    ...
    expect(opScreen['strip'].soldiers).toBe(true);

    const coa = new CoaScreen();
    expect(coa['strip'].soldiers).toBe(true);

    const debrief = new DebriefScreen({} as never);
    expect(debrief['strip'].soldiers).toBe(true);
