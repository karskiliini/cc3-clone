...
import { drawDarkPanel } from '@/ui/chrome';
...
export class CoaScreen implements Screen {
  ...
  constructor() {
    ...
    this.strip = new BottomStrip({ next: 'Briefing →', soldiers: !!game.campaign });
  ...
  update(dt: number, input: InputState): void {
    ...
    if (r.back) game.setScreen(new MainMenuScreen());
  ...
  draw(ctx: CanvasRenderingContext2D): void {
    drawMenuFrame(ctx, 'COURSE OF ACTION', () => this.drawBody(ctx));
  }
  ...
  private drawBody(ctx: CanvasRenderingContext2D): void {
    ...
    const sitText = wrapText(ctx, grandOp.situation, rw - 32);
