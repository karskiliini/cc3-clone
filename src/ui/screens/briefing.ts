...
import { BottomStrip, toMenuInput, beginMenuFrame, wordWrapCtx } from '@/ui/screens/common';
import { drawLogo, drawScreenTitle, drawShadowText, drawDarkPanel } from '@/ui/chrome';
...
export class BriefingScreen implements Screen {
  ...
  update(_dt: number, input: InputState): void {
    ...
    if (input.keysPressed.has('escape')) { game.setScreen(new CoaScreen()); return; }
    const m = toMenuInput(input);
    const r = this.strip.update(m);
    if (r.quitOrBack) game.setScreen(new CoaScreen());
  ...
  draw(ctx: CanvasRenderingContext2D): void {
    ...
    const opLines = wordWrapCtx(ctx, grandOp.situation, 360, '11px Arial, Helvetica, sans-serif');
    let y = 202;
    ...
    ctx.font = '11px Arial, Helvetica, sans-serif';
    const mapLines = wordWrapCtx(ctx, mapDef.description, 354, '11px Arial, Helvetica, sans-serif');
    let by = 202;
