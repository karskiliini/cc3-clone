...
export function drawLOSLine(
  ...
  heights?: LosHeights,
  opts?: { label?: boolean; alpha?: number; fireOrder?: boolean; minRangeM?: number | null },
): void {
  ...
  if (opts?.label ?? true) {
    drawAimRangeLabel(ctx, toPx, distM, estimated, endCls, opts?.minRangeM ?? null,
      context ? rangeColor(distM, teamMaxRangeM(context.state, context.team)) : CLASS_COLOR[endCls]);
  }
  ctx.restore();
}
...
export function drawAimRangeLabel(
  ...
  endCls: AimClass, minRangeM: number | null, color: string,
): void {
...
export function drawAimRangeFeedback(
  ...
  state: BattleState, team: Team, kind: string | null, endCls: AimClass = 'clear',
): void {
  const distM = dist(from, to) * TILE_M;
  const minRangeM = kind === 'smoke' ? teamSmokeMinRangeM(state, team) : null;
  drawAimRangeLabel(ctx, worldToScreen(cam, to), distM, false, endCls, minRangeM, rangeColor(distM, teamMaxRangeM(state, team)));
}
