...
// refs11/12 (logical 1024x768): Chat/Options 2..26, zoom + circle ~36.5 r8,
...
const CHAT_R: Rect = { x: 2, y: 741, w: 24, h: 13 };
const OPTIONS_R: Rect = { x: 2, y: 755, w: 24, h: 13 };
...
export class BottomStrip {
  ...
  update(input: InputState): BottomStripAction | null {
    ...
    if (inRect(p, CHAT_R)) this.hover.add('chat');
    ...
    for (const c of input.clicks) {
      ...
      if (inRect(cp, CHAT_R)) return 'chat';
  ...
  draw(ctx: CanvasRenderingContext2D, state: BattleState, selectedTeam: Team | null, watchedSoldierId: number | null): void {
    drawHudButton(ctx, CHAT_R, 'Chat', { hot: this.hover.has('chat'), fontKind: 'mini' });
