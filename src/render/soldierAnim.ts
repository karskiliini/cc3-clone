...
export function actionFor(s: Soldier, time: number, posture: Posture = postureFor(s, time), speedMps?: number): AnimAction {
  ...
  if (s.bandageUntil != null && time < s.bandageUntil) return 'bandage';
...
export function frameFor(s: Soldier, time: number, action: AnimAction, entry: AnimEntryInfo, speedMps?: number, mood: Mood = moodFor(s), gaitCycles?: number): number {
  ...
  if (GAITS.has(action)) {
    // accumulated cycles are counted against the default stride; the atlas's own rendered stride
    // (strideM) rescales them so the feet stay planted
    const strideFix = gaitCycles !== undefined && entry.strideM && entry.strideM > 0
      ? STRIDE_M[action as keyof typeof STRIDE_M] / entry.strideM : 1;
    const cycles = (gaitCycles !== undefined ? gaitCycles * strideFix : time * gaitCadence(action, speedMps, mood, entry.strideM)) + off;
  ...
  if (action === 'bandage') {
    ...
    return Math.floor(time * fps + off * n) % n;
  }
...
export function pickAnimation(s: Soldier, time: number, targetPos?: Vec2 | null, posture: Posture = postureFor(s, time), speedMps?: number): AnimPick {
  ...
  const face = action === 'bandage' ? s.bandageFace : undefined;
  ...
  const heading = face ? Math.atan2(face.x - s.pos.x, -(face.y - s.pos.y)) : headingFor(s, targetPos, speedMps, time);
  return { posture, action, mood, weapon, heading, keys, flinch };
}
