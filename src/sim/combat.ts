...
function stepMortarTeam(state: BattleState, rng: Rng, dt: number, team: Team, track: CombatTrack): void {
  ...
  state.projectiles.push({
    ...
    t0: state.time, flightS: isRocket ? 1.2 + distM / 120 : 3 + dist(gunner.pos, impact) * TILE_M / 60,
  ...
  if (isRocket) {
    // the round flies on its own; the HE burst lands when it lands (the grenade pattern) so the
    // visible flight and the burst agree on timing
    state.pendingBursts.push({
      at: state.time + 1.2 + distM / 120, pos: { ...impact }, weaponId: weapon.id, side: gunner.side, shooterId: gunner.id,
    });
  } else {
