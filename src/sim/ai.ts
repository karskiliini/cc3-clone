...
function stepAttackAI(state: BattleState, rng: Rng, battle: AIBattle, side: Side, mem: SideMemory): void {
  ...
  if (state.time < 60 && process.env.HARNESS) console.log(`[phase] t=${state.time.toFixed(0)} objectiveVL=${objectiveVL ? objectiveVL.id : null} men=${men} startMen=${plan.startMen} thr=${(plan.startMen * 0.3).toFixed(1)} phase=${plan.phase}`);
